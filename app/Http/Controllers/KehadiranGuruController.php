<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class KehadiranGuruController extends Controller
{
    public function index(Request $request)
    {
        $tahun  = $request->input('tahun', 'all'); // 'all' = semua tahun (default)
        $bulan  = $request->input('bulan');
        $search = $request->input('search');
        $isAll  = ($tahun === 'all');

        // ── STAT CARDS ────────────────────────────────────────────────
        $totalGuru = DB::table('fact_kinerja_guru as f')
            ->join('dim_guru as g', 'f.id_guru', '=', 'g.id_guru')
            ->join('dim_waktu as w', 'f.id_waktu', '=', 'w.id_waktu')
            ->when(!$isAll, fn($q) => $q->where('w.tahun', $tahun))
            ->when($bulan,  fn($q) => $q->where('w.bulan', $bulan))
            ->distinct('g.id_guru')
            ->count('g.id_guru');

        $agg = DB::table('fact_kinerja_guru as f')
            ->join('dim_waktu as w', 'f.id_waktu', '=', 'w.id_waktu')
            ->leftJoin('dim_absensi as da', 'f.id_absensi', '=', 'da.id_absensi')
            ->when(!$isAll, fn($q) => $q->where('w.tahun', $tahun))
            ->when($bulan,  fn($q) => $q->where('w.bulan', $bulan))
            ->selectRaw('
                SUM(da.jumlah_hadir) AS total_hadir,
                SUM(da.jumlah_sakit + da.jumlah_izin) AS total_izin,
                SUM(da.jumlah_alpha) AS total_alpha
            ')
            ->first();

        $totalHadir = $agg->total_hadir ?? 0;
        $totalIzin  = $agg->total_izin  ?? 0;
        $totalAlpha = $agg->total_alpha ?? 0;
        $totalHari  = 1; // placeholder, tidak digunakan langsung

        $guruHadir = DB::table('fact_kinerja_guru as f')
            ->join('dim_guru as g', 'f.id_guru', '=', 'g.id_guru')
            ->join('dim_waktu as w', 'f.id_waktu', '=', 'w.id_waktu')
            ->leftJoin('dim_absensi as da', 'f.id_absensi', '=', 'da.id_absensi')
            ->when(!$isAll, fn($q) => $q->where('w.tahun', $tahun))
            ->when($bulan,  fn($q) => $q->where('w.bulan', $bulan))
            ->where('da.jumlah_hadir', '>', 0)
            ->distinct('g.id_guru')
            ->count('g.id_guru');

        $guruIzin = DB::table('fact_kinerja_guru as f')
            ->join('dim_guru as g', 'f.id_guru', '=', 'g.id_guru')
            ->join('dim_waktu as w', 'f.id_waktu', '=', 'w.id_waktu')
            ->leftJoin('dim_absensi as da', 'f.id_absensi', '=', 'da.id_absensi')
            ->when(!$isAll, fn($q) => $q->where('w.tahun', $tahun))
            ->when($bulan,  fn($q) => $q->where('w.bulan', $bulan))
            ->whereRaw('(da.jumlah_sakit + da.jumlah_izin) > 0')
            ->distinct('g.id_guru')
            ->count('g.id_guru');

        $guruAlfa = DB::table('fact_kinerja_guru as f')
            ->join('dim_guru as g', 'f.id_guru', '=', 'g.id_guru')
            ->join('dim_waktu as w', 'f.id_waktu', '=', 'w.id_waktu')
            ->leftJoin('dim_absensi as da', 'f.id_absensi', '=', 'da.id_absensi')
            ->when(!$isAll, fn($q) => $q->where('w.tahun', $tahun))
            ->when($bulan,  fn($q) => $q->where('w.bulan', $bulan))
            ->where('da.jumlah_alpha', '>', 0)
            ->distinct('g.id_guru')
            ->count('g.id_guru');

        $totalStatus     = $totalHadir + $totalIzin + $totalAlpha;
        $persenKehadiran = $totalStatus > 0
            ? round(($totalHadir / $totalStatus) * 100, 2)
            : 0;

        // ── TREN KEHADIRAN PER BULAN ──────────────────────────────────
        // Saat isAll: group by tahun+bulan agar semua periode muncul
        $trenKehadiran = DB::table('fact_kinerja_guru as f')
            ->join('dim_waktu as w', 'f.id_waktu', '=', 'w.id_waktu')
            ->leftJoin('dim_absensi as da', 'f.id_absensi', '=', 'da.id_absensi')
            ->when(!$isAll, fn($q) => $q->where('w.tahun', $tahun))
            ->when($bulan,  fn($q) => $q->where('w.bulan', $bulan))
            ->selectRaw("
                w.bulan AS periode_bulan,
                " . ($isAll ? "w.tahun AS periode_tahun," : "") . "
                SUM(da.jumlah_hadir) AS total_hadir,
                SUM(da.jumlah_sakit + da.jumlah_izin) AS total_izin,
                SUM(da.jumlah_alpha) AS total_alpha,
                ROUND(
                    SUM(da.jumlah_hadir) * 100.0 /
                    NULLIF(
                        SUM(da.jumlah_hadir)
                        + SUM(da.jumlah_izin)
                        + SUM(da.jumlah_alpha),
                    0)
                , 2) AS persen_kehadiran
            ")
            ->groupBy($isAll ? ['w.tahun', 'w.bulan'] : ['w.bulan'])
            ->when($isAll, fn($q) => $q->orderBy('w.tahun'))
            ->orderBy('w.bulan')
            ->get();

        // ── RINGKASAN PER GURU (tabel utama) ─────────────────────────
        $ringkasanQuery = DB::table('fact_kinerja_guru as f')
            ->join('dim_guru as g', 'f.id_guru', '=', 'g.id_guru')
            ->join('dim_waktu as w', 'f.id_waktu', '=', 'w.id_waktu')
            ->leftJoin('dim_absensi as da', 'f.id_absensi', '=', 'da.id_absensi')
            ->when(!$isAll, fn($q) => $q->where('w.tahun', $tahun))
            ->when($bulan,  fn($q) => $q->where('w.bulan', $bulan))
            ->when($search, fn($q) => $q->where('g.nama_guru', 'like', "%{$search}%"))
            ->selectRaw("
                g.id_guru,
                g.nama_guru,
                g.nip,
                g.jabatan,
                COUNT(CASE WHEN da.jumlah_hadir > 0 THEN 1 END)
                    AS total_hadir_kerja,
                COUNT(CASE WHEN (da.jumlah_sakit + da.jumlah_izin) > 0 THEN 1 END)
                    AS total_izin,
                COUNT(CASE WHEN da.jumlah_alpha > 0 THEN 1 END)
                    AS total_alfa,
                (
                    COUNT(CASE WHEN da.jumlah_hadir > 0 THEN 1 END)
                    + COUNT(CASE WHEN (da.jumlah_sakit + da.jumlah_izin) > 0 THEN 1 END)
                    + COUNT(CASE WHEN da.jumlah_alpha > 0 THEN 1 END)
                ) AS total_hari_kerja,
                ROUND(
                    COUNT(CASE WHEN da.jumlah_hadir > 0 THEN 1 END) * 100.0
                    / NULLIF(
                        COUNT(CASE WHEN da.jumlah_hadir > 0 THEN 1 END)
                        + COUNT(CASE WHEN (da.jumlah_sakit + da.jumlah_izin) > 0 THEN 1 END)
                        + COUNT(CASE WHEN da.jumlah_alpha > 0 THEN 1 END),
                    0)
                , 2) AS persen_kehadiran_guru
            ")
            ->groupBy('g.id_guru', 'g.nama_guru', 'g.nip', 'g.jabatan')
            ->orderBy('g.nama_guru');

        $ringkasanGuru = $ringkasanQuery->paginate(15)->withQueryString();

        // ── FILTER OPTIONS ────────────────────────────────────────────
        $daftarTahun = DB::table('dim_waktu')
            ->select('tahun')->distinct()->orderByDesc('tahun')->pluck('tahun');

        // Daftar bulan menyesuaikan: kalau isAll ambil semua bulan, kalau tidak filter per tahun
        $daftarBulan = DB::table('dim_waktu')
            ->select('bulan')
            ->when(!$isAll, fn($q) => $q->where('tahun', $tahun))
            ->distinct()->orderBy('bulan')->pluck('bulan');

        $namaBulan = [
            1 => 'Januari',   2 => 'Februari', 3 => 'Maret',
            4 => 'April',     5 => 'Mei',       6 => 'Juni',
            7 => 'Juli',      8 => 'Agustus',   9 => 'September',
            10 => 'Oktober', 11 => 'November', 12 => 'Desember',
        ];

        return view('kehadiran-guru', compact(
            'tahun', 'isAll',
            'bulan', 'search',
            'totalGuru', 'guruHadir', 'guruIzin', 'guruAlfa',
            'persenKehadiran',
            'totalHadir', 'totalIzin', 'totalAlpha', 'totalHari',
            'trenKehadiran', 'ringkasanGuru',
            'daftarTahun', 'daftarBulan', 'namaBulan',
        ));
    }
}